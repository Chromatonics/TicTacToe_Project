...
for (i=0; i<xArray.length;i++) {
    ...
    setTimeout (function() {writeMsg(txtl);},1000);
}
    if (pOne>pTwo) {
        first="Player 1";
...
function check4Tie () {
    ...
    if (boardState.length>=9 &&check != "Three in a row, Player 1 wins!" &&check != "Three in a rpw, player 2 wins!") {
        var txt1 = "Nobody wins, it's a tie!";
        tieSound();
        writeMsg(txt1);
...
function glowBoard(pos) {
    ...
    for (var i=0; i < squares.length; i++) {
        if (i==index0) {
            ...
            var bg2 = squares[i];
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(122,244,66)';},100);
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(197,244,66)';},200);
            ...
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(122,244,235)';},700);
...
function blink() {
    var body = document.getElementById('body');
            ...
            setTimeout(function() {body.style.backgroundColor = '#f73581';},700);
            ...
            setTimeout(function() {body.style.backgroundColor = '#white';},1100);
}
...
function checkWinCon1 (info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0)=="0") {
            var match3Avatar = info[i].charAt(i);
        }
    ...
    if (match0Avatar !=undefined && match1Avatar !=undefined && match2Avatar !=undefined) {
        if (match0Avatar==match1Avatar==match2Avatar) {
            winDetected = "win";
        }
    }
...
function animateO(selected) {
    selected.style.transform = (selected.style.transform == "translateY(-100%)" ||null) ?
